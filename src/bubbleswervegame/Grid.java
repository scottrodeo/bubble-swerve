...
public class Grid {
	...
	public static int HEIGHT = 22;	// default height of game grid in bubbles
	public static int WIDTH = 12;	// default width of game grid in bubbles
	...
	private void initializeGrid() {  
		...
		board = new Bubble[Grid.HEIGHT][Grid.WIDTH];
	...
	private void deleteRowUp(int row) {
	    for (int r = row; r < Grid.HEIGHT - 1; r++) { // Shift rows up
	        for (int col = 0; col < Grid.WIDTH; col++) {
	    ...
	    for (int col = 0; col < Grid.WIDTH; col++) {
	        board[Grid.HEIGHT - 1][col].setColor(Grid.EMPTY);
	    }
	}

	
	private void deleteColumnLeft(int col) {
	    for (int c = col; c < Grid.WIDTH - 1; c++) { // Shift columns left
	        for (int row = 0; row < Grid.HEIGHT; row++) {
	    ...
	    for (int row = 0; row < Grid.HEIGHT; row++) {
	        board[row][Grid.WIDTH - 1].setColor(Grid.EMPTY);
	    }
	}
	...
	private void deleteColumnRight(int col) {
	    ...
	    for (int row = 0; row < Grid.HEIGHT; row++) {
	        board[row][0].setColor(Grid.EMPTY);
	    }
	}
	...
	public void draw(Graphics g) {
	    ...
	    int boardWidth = Grid.WIDTH * engine.getPanelGame().getCellSize();
	    int boardHeight = Grid.HEIGHT * engine.getPanelGame().getCellSize();
